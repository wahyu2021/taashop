            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                ...
                {news.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                        {news.map((item) => (
                            <div key={item.id} className="group flex flex-col h-full bg-white border border-stone-100 hover:border-stone-200 transition-all">
                                ...
                                <div className="p-6 md:p-6 lg:p-8 flex flex-col grow">
                                    ...
                                    <h3 className="text-lg lg:text-xl font-black uppercase tracking-tight text-stone-900 mb-4 line-clamp-2 leading-tight group-hover:text-orange-600 transition-colors">
                                        <Link href={`/news/${item.slug}`}>
                                            {item.title}
                                        </Link>
                                    </h3>
                                    <p className="text-stone-500 text-xs lg:text-sm leading-relaxed mb-6 line-clamp-3 grow">
                                        {item.summary}
                                    </p>                                    <Link 
                                        href={`/news/${item.slug}`}
                    ...
                    <EmptyState 
                        ...
                        message="Kami sedang merangkum berbagai informasi menarik untuk Anda. Nantikan update berita dan artikel dari kami segera."
                        icon={FileText}
